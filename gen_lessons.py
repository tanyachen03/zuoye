...
lessons_data = {
    'module1': {
        ...
        'count': 5,
        'lessons': [
            ('数据分析行业前景与岗位介绍', '了解数据分析行业的发展趋势、岗位需求和薪资水平'),
            ('数据分析思维', '培养数据分析思维，掌握业务分析方法'),
            ('Python极简入门', '变量、列表、字典、循环、函数基础'),
            ('Anaconda与Jupyter环境', '安装配置Anaconda和Jupyter Notebook'),
            ('在线编程平台使用', '浏览器中编写和运行Python代码'),
        ]
    },
    'module2': {
        'name': '模块二：Pandas核心基础',
        ...
        'count': 7,
        'lessons': [
            ('Pandas与Series', 'Pandas库介绍和Series数据结构'),
            ('DataFrame结构', '掌握DataFrame二维表格结构'),
            ('读取各类数据', 'Excel、CSV、在线数据集读取'),
            ('数据查看方法', 'head、tail、info、describe'),
            ('行列选取与筛选', 'loc、iloc、布尔索引'),
            ('缺失值与重复值', '处理缺失值和重复数据'),
            ('类型转换与重命名', '数据类型转换和字段重命名'),
        ]
    },
    'module3': {
        'name': '模块三：数据清洗与预处理',
        ...
        'count': 7,
        'lessons': [
            ('异常值识别与处理', 'IQR法、Z-score法'),
            ('字符串数据清洗', '拆分、替换、提取'),
            ('时间日期处理', 'pd.to_datetime、日期提取'),
            ('新增字段与分箱', '计算字段、cut、qcut'),
            ('多表合并', 'merge、concat'),
            ('分组聚合groupby', 'sum、mean、count、agg'),
            ('透视表实战', 'pivot_table多维分析'),
        ]
    },
    'module4': {
        'name': '模块四：数据可视化分析',
...
def make_nav(mod_key, mod_data, current):
    nav = ''
    for j in range(1, mod_data['count'] + 1):
        active = 'active' if j == current else ''
        title = mod_data['lessons'][j-1][0]
        nav += '<a href="lesson' + str(j) + '.html" class="lesson-nav-item ' + active + '"><span class="nav-number">' + str(j) + '</span><span class="nav-title">' + title + '</span></a>'
    return nav
...
<aside class="s">
<div class="sc">
<div class="st">MOD_NAME</div>
NAV_HERE
</div>
</aside>
<main class="mc">
<div class="lh">
<div class="lm"><span class="mt">MOD_TAG</span></div>
<h1>LESSON_TITLE</h1>
<p>LESSON_DESC</p>
</div>
<div class="se">
...
let editor, pyodide;
function initEditor(){
editor=CodeMirror(document.getElementById('code-editor'),{mode:'python',theme:'monokai',lineNumbers:true,tabSize:4,value:"# LESSON_TITLE\nprint('欢迎学习数据分析!')"});
}
...
function resetCode(){editor.setValue("# LESSON_TITLE\nprint('欢迎学习数据分析!')");document.getElementById('output').textContent=''}
function markComplete(){
const c=JSON.parse(localStorage.getItem('completedLessons')||'{}');
c['LESSON_ID']=true;
localStorage.setItem('completedLessons',JSON.stringify(c));
...
const c=JSON.parse(localStorage.getItem('completedLessons')||'{}');
if(c['LESSON_ID']){const b=document.getElementById('completeBtn');b.innerHTML='<i class="fas fa-check-circle"></i> 已完成';b.disabled=true}});
</script>
</body>
</html>'''

for mod_key, mod_data in lessons_data.items():
    for i, (title, desc) in enumerate(mod_data['lessons'], 1):
        lesson_id = 'm' + str(int(mod_key[-1])) + 'l' + str(i)
        html = base_html.replace('TITLE', title)
        html = html.replace('MOD_NAME', mod_data['name'])
        html = html.replace('MOD_TAG', mod_data['tag'])
        html = html.replace('LESSON_TITLE', title)
        html = html.replace('LESSON_DESC', desc)
        html = html.replace('LESSON_ID', lesson_id)
        html = html.replace('NAV_HERE', make_nav(mod_key, mod_data, i))
        
        filepath = '/workspace/data-analytics-platform/course/' + mod_key + '/lesson' + str(i) + '.html'
