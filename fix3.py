...
# Find the start of the corrupted make_chapter(1, ...) call
start_marker = "            make_chapter(1, 'Python 基础与环境搭建', '15 分钟',"
start_idx = data.find(start_marker)
if start_idx == -1:
    print("ERROR: Could not find start marker")
    exit(1)

# Find the END of this first make_chapter call
# It should end with the starter_code string followed by '),'
# We look for the pattern: ...print("均值:", arr.mean())'),  <-- next make_chapter starts after this
end_marker = "            make_chapter(2, 'NumPy 数值计算', '25 分钟',"
end_idx = data.find(end_marker)
if end_idx == -1:
    print("ERROR: Could not find end marker (make_chapter 2)")
    exit(1)

# The corrupted section is from start_idx to end_idx
corrupted_section = data[start_idx:end_idx]
print(f"Corrupted section length: {len(corrupted_section)}")
print(f"Corrupted section starts with: {repr(corrupted_section[:80])}")
print(f"Corrupted section ends with: {repr(corrupted_section[-80:])}")

# Build the correct replacement
# The new_chapter1 should properly close all strings and the make_chapter call
# Using double-quoted strings for the code parts to avoid any single-quote issues
new_chapter1 = """            make_chapter(1, 'Python 基础与环境搭建', '15 分钟',
...
print(f"New chapter 1 length: {len(new_chapter1)}")

# Do the replacement
new_data = data[:start_idx] + new_chapter1 + data[end_idx:]
